...
public class HadoopS3RecoverableWriterTest extends TestLogger {
	...
	public void prepare() throws Exception {
		...
		if (!Files.exists(path)) {
			...
			try (Stream<java.nio.file.Path> files = Files.list(path)) {
				files.forEach(p -> {
					try {
						Files.delete(p);
					} catch (IOException e) {
	...
	public void cleanupAndCheckTmpCleanup() throws Exception {
		final java.nio.file.Path localTmpDir = Paths.get(fileSystem.getLocalTmpDir());
		Assert.assertTrue(Files.exists(localTmpDir));

		Assert.assertEquals(0L, Files.list(localTmpDir).count());
		fileSystem.delete(basePathForTest, true);
	}

	private FlinkS3FileSystem getFileSystem() throws Exception {
		if (fileSystem == null) {
